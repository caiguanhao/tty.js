...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(cols, rows, params, func) {
    var conf = session.server.conf;
    if (conf.beforeCreate && typeof conf.beforeCreate === 'function') {
      conf.beforeCreate.call(session, params, function(beforeCreate) {
        params.beforeCreate = beforeCreate;
        session.handleCreate(cols, rows, params, func);
      });
    } else {
      session.handleCreate(cols, rows, params, func);
    }
...
Session.prototype.handleCreate = function(cols, rows, params, func) {
  var self = this
    , terms = this.terms
  ...
  if (conf.runScriptOnStart && typeof conf.runScriptOnStart === 'function') {
    conf.runScriptOnStart(term, params);
  }
